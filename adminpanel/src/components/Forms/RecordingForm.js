...
class RecordingForm extends React.Component{
   ...
   componentDidMount() {
    fetch(`/api/recording/${this.props.date}`).then(res => res.json()).then(data => this.setState({
   ...
   handleSubmit(event) {
       this.setState({show: false});
        ...
        fetch(`/api/recording/${this.props.date}`, fetchOptions).then((res) => (window.location.reload())).catch((err) => (alert('Error')))
   }
